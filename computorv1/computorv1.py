...
from ma_class import polynome_class
...
def clean_tab(classe):
    while (len(classe.tab) > 1 and classe.tab[len(classe.tab) - 1] == 0):
        del classe.tab[len(classe.tab) - 1]
...
def main():
    ...
    for arg in ma_classe.tab:
        if i > 2:
            print(Fore.RED, end = '')
        if len(sys.argv) == 3 and sys.argv[2].find('z') != -1 and arg == 0:
            pass
        else:
            if i != 0:
                if arg >= 0:
    ...
    if len(ma_classe.tab) == 3:
        ...
        if ma_classe.tab[0] == 0:
            ...
            ret = ma_classe.tab[1] / (ma_classe.tab[0]) * - 1
