...
/*   Updated: 2019/04/09 15:01:12 by akhercha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		fill_champ(t_vm *vm, char *cor_file, int *id, int cli_flag)
{
	...
	if (!ft_file_exists(cor_file))
		free_and_quit(vm, "Error: file .cor not found.", EXIT_FAILURE);
	if (champ_id_exists(vm->champs, *id))
	{
		if (cli_flag)
			free_and_quit(vm, "Error: duplicated id.", EXIT_FAILURE);
