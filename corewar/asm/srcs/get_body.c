...
/*   Updated: 2019/04/10 19:54:58 by xgilbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_args(t_asm *tasm, int i, int n, int k)
{
	...
	if (tasm->file[i][k - 2] != '\n')
		tasm->instr[n]->args[a - 1] = ft_strjoin1(tasm->instr[n]->args[a - 1],
				&tasm->file[i][k - 1]);
//	ft_printf("ce qu il y a dans l arg : %s",tasm->instr[n]->args[a - 1]);
	tasm->instr[n]->nb_arg = a;
	while (a < 3)
		tasm->instr[n]->args[a++] = NULL;
}
