...
dimension (number of features + 1, 1).
    ...
    def fit_(self, X, Y, alpha = 0.005, n_cycle = 1):
        ...
        for i in range(n_cycle):
            ...
            bad_predict = np.reshape(bad_predict,(Y.shape))
            # print("X",X.shape)
            # print("Y", Y.shape)
            # print("bad predict", bad_predict.shape)
            # print((Y - bad_predict).shape)
            # print(Y - bad_predict)
            D_m = (-2/n) * sum(X * (Y - bad_predict))  # Derivative wrt m
            # print("Y apres ", Y.shape)
            # print("X apres D_m",X.shape)

            # print("D_m ",D_m.shape)
            D_c = (-2/n) * sum(Y - bad_predict)  # Derivative wrt c
            # print("D_c", D_c.shape)

            D_m = np.reshape(D_m,(self.theta[1:].shape))
            
            
            # print("theta [1:]",self.theta[1:].shape)
            # print("D_m",D_m.shape)
            # print("return (alpha * D_m) ", (alpha * D_m).shape )
